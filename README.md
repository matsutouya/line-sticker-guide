...
2. **「濃い輪郭線を壁にして、背景から線を越えずに到達できる画素を消す」**。顔アップなど体が画面端で切れている構図では輪郭が閉じていないため、そこから浸水して明るい毛とお腹を全部たどり、キャラクターが輪郭線だけになる。輪郭線が閉じた壁になっているという前提が成り立たない
...
**正しい直し場所は、コマを切り出す段階**(5-4)。そこでは影はまだ白背景と地続きで、お腹のクリームは濃い輪郭線に囲まれて外周から切り離されている。だから`remove_white_bg()`の判定を「外周と連結した、明るく低彩度の領域」に差し替えれば、影だけが落ちてお腹は残る。base.jpgの足元の影を落とすのと同じ理屈。
